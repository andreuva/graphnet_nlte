...
class Dataset3D(torch.utils.data.Dataset):
    def __init__(self, hyperparameters, datadir='../data/', prefix='train'):
        """
        Dataset for the 3D case.
        """
        super(Dataset, self).__init__()

        # Read the data from the files

        # Now we need to define the graph for the 3D model.
        # The graph will connect all points at certain distance. We define this distance
        # as integer indices, so that we make sure that nodes are connected to the neighbors

        # Select the number of subcubes to extract from the main 3D cube
        self.n_samples = 1

        # Initialize the graph information
        self.edge_index = [None] * self.n_samples
        self.nodes = [None] * self.n_samples
        self.edges = [None] * self.n_samples
        self.u = [None] * self.n_samples
        self.target = [None] * self.n_samples
        ...
        for i in tqdm(range(self.n_samples)):
            ...
            nz = 3
            num_nodes = nx*ny*nz
            index_tau = np.zeros((num_nodes, 3))

            index_tau[:, 0] = np.arange(nx)
            index_tau[:, 1] = np.arange(ny)
            index_tau[:, 2] = np.arange(nz)
